...
@property(nonatomic,strong) KNdrawView * drawV;
...
{
    if (_drawV == nil) {
        _drawV = [[KNdrawView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height)];
...
//    UIAlertView *alertView=[[UIAlertView alloc]initWithTitle:@"提示" message: @"确定要保存至手机相册吗?"delegate:nil cancelButtonTitle:@"取消"otherButtonTitles:@"确定", nil];
    ...
    //初始化提示框；
      UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"提示" message:@"确定要保存至手机相册?" preferredStyle:  UIAlertControllerStyleAlert];
    ...
    __weak __typeof__(self) weakSelf = self;
      ...
      [alert addAction:[UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        //点击按钮的响应事件；
          ...
          UIImageWriteToSavedPhotosAlbum(self->image, weakSelf, @selector (image:didFinishSavingWithError:contextInfo:), nil);
